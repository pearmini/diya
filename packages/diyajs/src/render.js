...
function tree(spec) {
  ...
  const root = hierarchy({root: data}, children);

  root.each(bind).each(flex);
  ...
  function bind(d) {
    ...
    const {label = key, visible = true, ...restConfig} = config[key] || {};
    d.data = isPlainObject(data) ? data : {};
    Object.assign(d.data, {key, label, visible, ...restConfig});
  }
...
function renderJSON(spec, options) {
  const {width, height, padding = 0, paddingTop = padding * 2 + labelHeight()} = spec;
  const {treemap: t = treemap()} = options;
  ...
  const root = t
    ...
    .paddingLeft(visible("paddingLeft", padding))(tree(spec));
